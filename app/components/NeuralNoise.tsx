...
export function NeuralNoise({ isChatMode = false, currentLook = 'default' }: NeuralNoiseProps) {
  ...
  // Named renderFrame (not render) - eslint's react-hooks/purity rule flags
  // impure calls (performance.now()) inside any function literally named
  // "render" as a heuristic for "might run during React's render phase."
  // This is a rAF-driven WebGL animation tick, unrelated to component
  // rendering, so the naming collision is a false positive.
  const renderFrame = () => {
    const gl = glRef.current;
    ...
    // renderFrame only ever runs from a requestAnimationFrame callback (see
    // below), never during React's render phase - the linter's static
    // analysis can't see that call-site guarantee from here, the same gap
    // Scene's camera-positioning effect in R3FVisualizer.tsx works around
    // with an exhaustive-deps disable comment for an analogous reason.
  ...
  useEffect(() => {
    // Only run on client side to prevent hydration mismatch. Deferred (not
    // a synchronous setState-in-effect) so React doesn't cascade a render
    // while this effect is still committing - same pattern as useMicLevel.ts.
  ...
  useEffect(() => {
    ...
    // render, and re-running this effect on their identity would tear down
    // and rebuild the whole WebGL context every render for no reason.
